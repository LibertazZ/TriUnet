...
net = FURENet_test1(1)
net.load_state_dict(torch.load("/media/disk8T/zhn/Q3/log2/20.pth")['net'])

zin, zdr, rain = val_dataset[j]

zin, zdr, rain= torch.from_numpy(zin).unsqueeze(0), torch.from_numpy(zdr).unsqueeze(0), torch.from_numpy(rain).unsqueeze(0)
