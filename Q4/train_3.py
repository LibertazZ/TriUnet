...
model_dir = '/media/disk8T/zhn/Q4/log_3'
...
def train(epoch):
    ...
    net.train()
    total = 0
    for batch_idx, (zin, zdr,kdp,rain) in enumerate(trainloader):
        # lr = learning_rate(epoch + (batch_idx+1)/len(trainloader))  # 闅廱atch_idx鏀瑰彉
        # print(lr)
        # optimizer.param_groups[0].update(lr=lr)
        zin, zdr,kdp,rain = zin.cuda(), zdr.cuda(),kdp.cuda(),rain.cuda()
        optimizer.zero_grad()
...
def test(epoch):
    ...
    for batch_idx, (zin, zdr,kdp,rain) in enumerate(valloader):
        # lr = learning_rate(epoch + (batch_idx+1)/len(trainloader))  # 闅廱atch_idx鏀瑰彉
        # print(lr)
        # optimizer.param_groups[0].update(lr=lr)
        zin, zdr,kdp,rain = zin.cuda(), zdr.cuda(),kdp.cuda(),rain.cuda()
