...
net = FURENet_3(1)
net.load_state_dict(torch.load("/media/disk8T/zhn/Q4/log_3/90.pth")['net'])

zin, zdr,kdp,rain = val_dataset[j]

zin, zdr,kdp,rain =  torch.from_numpy(zin).unsqueeze(0),  torch.from_numpy(zdr).unsqueeze(0),  torch.from_numpy(kdp).unsqueeze(0), torch.from_numpy(rain).unsqueeze(0)
...
# 定义颜色区间和颜色映射
color_intervals = np.arange(0, 121, 10)  # 定义颜色区间为[0-10]和[10-20]
